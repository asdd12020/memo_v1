    ...
    public string tempName;
        protected override void OnMouseUp(MouseEventArgs e)
        {
            ...
            if (this.AllowClose && pointedTab != null)
            {
                ...
                {
                    //這邊還要再修改成自己做的ipnut...不然按取消會沒值
                    //還要做新增標籤的東西
                    //contextmenuTab = pointedTab;
                    //string input = Microsoft.VisualBasic.Interaction.InputBox("請輸入新的Tab Name", "重新命名", contextmenuTab.Text, -1, -1);
                    //contextmenuTab.Text = input;
                    
                    //f2.Show();
                    if (f2.ShowDialog() == DialogResult.OK)
                    {
                       tempName = f2.Msg;
                       pointedTab.Text = tempName;
                    }
                    //System.Windows.Forms.MessageBox.Show(tempName);
                }
            }
            base.OnMouseUp(e);
        }
        ...
        private TabPage AddTab(Point position)
        {
            for (int i = 0; i < this.TabPages.Count; i++)
                if (this.GetTabRect(i).Contains(position))
                    return this.TabPages[i];
